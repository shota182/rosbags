...
def plot_csv_data(csv_file, start_sec=0.0, end_sec=None, gradient_threshold=10.0):
    ...
    colorlist = ["salmon", "red", "blue", "orange", "pink", "steelblue", "yellow", "cyan"]
    ...
    # プロット
    plt.plot(df_range["relative_time"].to_numpy(), data_raw[2], label=r"$\theta_{l}$", color=colorlist[1], lw=15)
    plt.plot(df_range["relative_time"].to_numpy(), data_raw[7], label=r"$\theta_{f}$", color=colorlist[4], lw=10)
    ...
    # plt.legend(fontsize=60, loc='center left', bbox_to_anchor=(1.02, 0.5), borderaxespad=0)
    plt.grid()
    plt.show()
